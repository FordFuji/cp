...
function discountPercent($product_before_discount_price, $product_price) {
	$price = $product_before_discount_price - $product_price;
	
	return ceil($price * 100 / $product_before_discount_price);
}

function checkPrice($product_before_discount_price, $product_price) {
	if($product_before_discount_price == $product_price) {
		return '';
	} else {
		return '<div><s>THB '.number_format($product_before_discount_price, 0, '.', ',').'</s></div>';
	}
}

function checkPriceProductDetail($product_before_discount_price, $product_price) {
	if($product_before_discount_price == $product_price) {
		return '';
	} else {
		return '<div class="full-price">THB'.number_format($product_before_discount_price, 0, '.', ',').'</div>';
	}
}

function getProductRoute($product_name) {
...
function date2DateEnBasic($datetime) {
	...
	return $day.'.'.$month.'.'.$year.' '.$hour_.':'.$minute.' น.';
}

function date2DateFormat($datetime) {
	$datetime_exp = explode(' ', $datetime);
	...
	return $datetime_exp[0];
	//return $month.'/'.$day.'/'.$year;
}

function date2HourFormat($datetime) {
	$datetime_exp = explode(' ', $datetime);
	$time_exp = explode(':', $datetime_exp[1]);
...
function date2MinuteFormat($datetime) {
	$datetime_exp = explode(' ', $datetime);
	$time_exp = explode(':', $datetime_exp[1]);
