...
class Backend extends CI_Controller {
	...
	public function __construct() {
		...
		$this->load->model('training/model_training');
		$this->load->model('training/model_training_datatable');
		$this->load->model('template_main/model_template_main');
		//$this->load->library('datatables');
		$this->load->library('table');
		$this->load->helper('form');
		$this->path_upload = FCPATH.'uploads/training/';
	...
	public function server_processing() {
        ...
        $results = $this->model_training_datatable->training_datatable($param);
	...
	public function form($id = ''){	
		$data['id'] = $id;
		$data['row'] = $this->model_training->get_data_single($id);

		$data['profile'] = $this->model_training->getProfileList();
		...
		$this->load->view('training/training/form', $data);
	}
	...
	public function save_update($id = ''){	
		...
		/*if(!empty($_FILES['training_image'])) {
			...
			if($this->upload->do_upload('training_image')) {
				...
				$config_resize['source_image'] = FCPATH.'uploads/training/'.$data_image['file_name'];
				$config_resize['new_image'] = FCPATH.'uploads/training/'.$data_image['file_name'];
				...
				$data['training_image'] = $data_image['file_name'];
			} else {
		...
		// update 
		if($id != '') {	
			$this->model_training->update_data($data, $id);
			
			redirect('training/backend/index', 'location');
			
		// insert
		} else {	
			$data['training_datetime_create'] = date('Y-m-d H:i:s');
				
			$this->model_training->insert_data($data);
	...
	public function delete($id){
		$this->model_training->delete_data($id);
