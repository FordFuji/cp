...
class Backend extends CI_Controller {
	...
	public function __construct() {
		...
		$this->load->model('contact_us/model_contact_us_datatable');
	...
	public function index() {
		...
		/* start body */
		$this->load->view('contact_us/contact_us/list', $data);
		/* end body */
	}
	...
	public function server_processing() {
        ...
        $results = $this->model_contact_us_datatable->contact_us_datatable($param);
	...
	/*public function form($id = ''){	
		$data['id'] = $id;
		$data['row'] = $this->model_contact_us->get_data_single($id);
	...
	public function save_update($id = ''){	
		$data = array(
			...
			'contact_us_select' =>  $this->input->post('contact_us_select'),
			'contact_us_ckeditor' =>  $this->input->post('contact_us_ckeditor'),
			'contact_us_username_update' => $this->session->userdata('session_username'),
			'contact_us_datetime_update' => date('Y-m-d H:i:s'),
			'contact_us_ip_update' => $_SERVER['REMOTE_ADDR']
		);
		
		if(!empty($_FILES['contact_us_image'])) {
			$config['upload_path']          = FCPATH.'uploads/contact_us/';
			...
			if($this->upload->do_upload('contact_us_image')) {
				...
				$config_resize['source_image'] = FCPATH.'uploads/contact_us/'.$data_image['file_name'];
				$config_resize['new_image'] = FCPATH.'uploads/contact_us/'.$data_image['file_name'];
				...
				$data['contact_us_image'] = $data_image['file_name'];
			} else {
		...
		// update 
		if($id != '') {	
			$this->model_contact_us->update_data($data, $id);
			
			redirect('contact_us/backend/index', 'location');
			
		// insert
		} else {	
			$data['contact_us_username_create'] = $this->session->userdata('session_username');
			$data['contact_us_datetime_create'] = date('Y-m-d H:i:s');
			$data['contact_us_ip_create'] = $_SERVER['REMOTE_ADDR'];
				
			$this->model_contact_us->insert_data($data);
	...
	public function delete($id){
		$this->model_contact_us->delete_data($id);
